...
import (
	...
	"github.com/itchenyi/common/rest"
)
...
func Example_get() {
	resp, err := rest.R().Get("http://httpbin.org/get")
...
func Example_post() {
	...
	resp2, err2 := rest.R().
		SetBody(rest.User{Username: "testuser", Password: "testpass"}).
		SetResult(&AuthSuccess{}). // or SetResult(AuthSuccess{}).
...
func Example_dropboxUpload() {
	...
	// See we are not setting content-type header, since rest automatically detects Content-Type for you
	resp, err := rest.R().
		SetBody(fileBytes).     // rest autodetects content type
...
func Example_put() {
	...
	resp, err := rest.R().
		SetBody(Article{
			...
			Tags:    []string{"article", "sample", "rest"},
		}).
...
func Example_socks5Proxy() {
	...
	// set transport into rest
	rest.SetTransport(ptransport)

	resp, err := rest.R().Get("http://check.torproject.org")
	fmt.Println(err, resp)
}

func printOutput(resp *rest.Response, err error) {
