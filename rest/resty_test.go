...
func TestPostJSONStringSuccess(t *testing.T) {
	...
	c.SetHeader(hdrContentTypeKey, jsonContentType).
		SetHeaders(map[string]string{hdrUserAgentKey: "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_5) rest 1.0", hdrAcceptKey: jsonContentType})
...
func TestPostJSONBytesSuccess(t *testing.T) {
	...
	c := dc()
	c.SetHeader(hdrContentTypeKey, jsonContentType).
		SetHeaders(map[string]string{hdrUserAgentKey: "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_5) rest v0.7", hdrAcceptKey: jsonContentType})
...
func TestOptionsMethod(t *testing.T) {
	...
	assertError(t, err)
	assertEqual(t, http.StatusOK, resp.StatusCode())
	assertEqual(t, resp.Header().Get("Access-Control-Expose-Headers"), "x-go-rest-id")
}
...
func TestOutputFileWithBaseDirAndRelativePath(t *testing.T) {
	...
	resp, err := R().
		SetOutput("rest/test-img-success.png").
...
func TestOutputFileWithBaseDirError(t *testing.T) {
	c := dc().SetRedirectPolicy(FlexibleRedirectPolicy(10)).
		SetOutputDirectory(getTestDataPath() + `/rest\0`)
...
func TestOutputFileAbsPath(t *testing.T) {
	ts := createGetServer(t)
	defer ts.Close()
	defer cleaupFiles("test-data/rest")

	_, err := dcr().
		SetOutput(getTestDataPath() + "/rest/test-img-success-2.png").
		Get(ts.URL + "/my-image.png")
...
func createGenServer(t *testing.T) *httptest.Server {
	ts := createTestServer(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == MethodOptions && r.URL.Path == "/options" {
			...
			w.Header().Set("Access-Control-Expose-Headers", "x-go-rest-id")
