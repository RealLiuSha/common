...
var (
	...
	hdrUserAgentValue = "rest v%s - https://github.com/itchenyi/common/rest"
...
//		rest.SetHostURL("http://myjeeva.com")
//
//		// Setting HTTPS address
//		rest.SetHostURL("https://myjeeva.com")
//
...
// Also it can be overridden at request level header options, see `rest.R().SetHeader`
// or `rest.R().SetHeaders`.
//
...
// overridden at request level headers options, see `rest.R().SetHeaders` or `rest.R().SetHeader`.
//
// Example: To set `Content-Type` and `Accept` as `application/json`
//
...
// SetCookieJar method sets custom http.CookieJar in the rest client. Its way to override default.
// Example: sometimes we don't want to save cookies in api contacting, we can remove the default
// CookieJar in rest client.
//
//		rest.SetCookieJar(nil)
//
...
// 				})
//
//		// Setting a cookies into rest
// 		rest.SetCookies(cookies)
//
...
// see `rest.R().SetQueryParam` or `rest.R().SetQueryParams`.
// 		rest.
...
// see `rest.R().SetQueryParams` or `rest.R().SetQueryParam`.
// 		rest.SetQueryParams(map[string]string{
...
// this client instance. Also it can be overridden at request level form data, see `rest.R().SetFormData`.
// 		rest.SetFormData(map[string]string{
...
//
// Example: To set the header for username "rest" and password "welcome"
// 		rest.SetBasicAuth("rest", "welcome")
//
// This basic auth information gets added to all the request rasied from this client instance.
// Also it can be overridden or set one at the request level is supported, see `rest.R().SetBasicAuth`.
//
...
//
// 		rest.SetAuthToken("BC594900518B4F7EAC75BD37F019E08FBC594900518B4F7EAC75BD37F019E08F")
//
// This bearer auth token gets added to all the request rasied from this client instance.
// Also it can be overridden or set one at the request level is supported, see `rest.R().SetAuthToken`.
//
...
// Its gets applied after default `rest` request middlewares and before request
// been sent from `rest` to host server.
// 		rest.OnBeforeRequest(func(c *rest.Client, r *rest.Request) error {
...
// Once we receive response from host server, default `rest` response middleware
// gets applied and then user assigened response middlewares applied.
// 		rest.OnAfterResponse(func(c *rest.Client, r *rest.Response) error {
...
// SetPreRequestHook method sets the given pre-request function into rest client.
// It is called right before the request is fired.
//
// Note: Only one pre-request hook can be registered. Use `rest.OnBeforeRequest` for mutilple.
...
// SetDebug method enables the debug mode on `rest` client. Client logs details of every request and response.
...
// SetDisableWarn method disables the warning message on `rest` client.
// For example: rest warns the user when BasicAuth used on HTTP mode.
...
// SetAllowGetMethodPayload method allows the GET method with payload on `rest` client.
// For example: rest allows the user sends request with a payload on HTTP GET method.
...
// SetLogger method sets given writer for logging rest request and response details.
// Default is os.Stderr
// 		file, _ := os.OpenFile("/Users/jeeva/rest.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
//
//		rest.SetLogger(file)
//
...
// By default rest won't set `Content-Length`.
// 		rest.SetContentLength(true)
//
// Also you have an option to enable for particular request. See `rest.R().SetContentLength`
//
...
//		rest.SetTimeout(time.Duration(1 * time.Minute))
//
...
// SetError method is to register the global or client common `Error` object into rest.
...
// SetRedirectPolicy method sets the client redirect poilicy. rest provides ready to use
// redirect policies. Wanna create one for yourself refer `redirect.go`.
//
//		rest.SetRedirectPolicy(FlexibleRedirectPolicy(20))
//
// 		// Need multiple redirect policies together
//		rest.SetRedirectPolicy(FlexibleRedirectPolicy(20), DomainCheckRedirectPolicy("host1.com", "host2.net"))
//
func (c *Client) SetRedirectPolicy(policies ...interface{}) *Client {
	for _, p := range policies {
		if _, ok := p.(RedirectPolicy); !ok {
			c.Log.Printf("ERORR: %v does not implement rest.RedirectPolicy (missing Apply method)",
				functionName(p))
		}
	}
...
// SetRetryCount method enables retry on `rest` client and allows you
...
// SetHTTPMode method sets rest mode into HTTP
func (c *Client) SetHTTPMode() *Client {
	return c.SetMode("http")
}

// SetRESTMode method sets rest mode into RESTful
func (c *Client) SetRESTMode() *Client {
	return c.SetMode("rest")
}

// SetMode method sets rest client mode to given value such as 'http' & 'rest'.
// 	RESTful:
//		- No Redirect
...
//		rest.SetTLSClientConfig(&tls.Config{ RootCAs: roots })
//
// 		// or One can disable security check (https)
//		rest.SetTLSClientConfig(&tls.Config{ InsecureSkipVerify: true })
...
// SetProxy method sets the Proxy URL and Port for rest client.
//		rest.SetProxy("http://proxyserver:8888")
//
...
// RemoveProxy method removes the proxy configuration from rest client
//		rest.RemoveProxy()
//
...
// SetCertificates method helps to set client certificates into rest conveniently.
//
...
// SetRootCertificate method helps to add one or more root certificates into rest client
// 		rest.SetRootCertificate("/path/to/root/pemFile.pem")
//
...
// If the output directory not exists then rest creates one. This setting is optional one,
// if you're planning using absoule path in `Request.SetOutput` and can used together.
// 		rest.SetOutputDirectory("/save/http/response/here")
//
...
// compatible interface implementation in the rereststy client.
//
// Please Note:
//
// - If transport is not type of `*http.Transport` then you may not be able to
// take advantage of some of the `rest` client settings.
//
// - It overwrites the rest client transport instance and it's configurations.
//
...
// SetScheme method sets custom scheme in the rest client. It's way to override default.
// 		rest.SetScheme("http")
//
...
func (c *Client) execute(req *Request) (*Response, error) {
	...
	// to modify the *rest.Request object
